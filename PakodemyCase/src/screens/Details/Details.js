  ...
  const renderMovieDetails = () => {
    if (error) {
      ...
      return (
        <>
          {movie?.Poster === "N/A" ?
            (<Image source={require("../../assets/images/noPoster.png")} style={styles.image} />)
            :
          ...
          <View style={styles.infoContainer}>
            ...
            <Text style={styles.tabText}> {movie?.Released}<Icon2 name="dot-single" size={20} />{movie?.Runtime} <Icon2
              name="dot-single" size={20} />{movie?.Country} </Text>
          </View>
        </>
      );
    }
    ...
    if (shownMovies.length > 0) {
      return (
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              style={styles.movieItemContainer} onPress={() => handleGetDetailsShownMovie(item.movieId)}>
